...
import { ArrowUpRight, AtSign, Check, ChevronRight, Globe, Mail, Menu, Play, X } from 'lucide-react'
import { Drawer, DrawerClose, DrawerContent, DrawerDescription, DrawerFooter, DrawerHeader, DrawerTitle, DrawerTrigger } from '@/components/ui/drawer'
import { projects, services, testimonials } from '@/lib/site-data'

export function Logo() { return <Link href="/" className="font-mono text-lg font-bold tracking-[0.2em] text-primary">NEGO<span className="text-accent">.</span></Link> }

const navLinks = [['About', '/about'], ['Services', '/services'], ['Projects', '/projects'], ['Portfolio', '/portfolio'], ['Team', '/team'], ['Contact', '/contact']] as const
...
export function Reveal({ children, className = '', delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { setVisible(true); observer.disconnect() } }, { rootMargin: '0px 0px -10% 0px', threshold: 0.01 })
...
export function Header() {
  ...
  return <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
    <div className="container flex h-16 items-center justify-between sm:h-20"><Logo />
      <nav className="hidden items-center gap-7 md:flex" aria-label="Main navigation">{navLinks.map(([label, href]) => <Link key={href} href={href} className={`text-xs font-bold uppercase tracking-[0.12em] transition-colors hover:text-accent ${path === href ? 'text-accent' : 'text-foreground'}`}>{label}</Link>)}</nav>
      <Drawer open={open} onOpenChange={setOpen} swipeDirection="right">
        <DrawerTrigger render={<button className="inline-flex size-11 items-center justify-center border border-border md:hidden" aria-label="Open navigation menu" />}><Menu aria-hidden="true" /></DrawerTrigger>
        <DrawerContent className="w-[min(88vw,22rem)] bg-primary text-primary-foreground">
          <DrawerHeader className="border-b border-primary-foreground/15 p-6 text-left"><DrawerTitle className="font-display text-2xl text-primary-foreground">NEGO<span className="text-accent">.</span></DrawerTitle><DrawerDescription className="text-left text-primary-foreground/60">Construction and engineering, delivered with clarity.</DrawerDescription></DrawerHeader>
          <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-6" aria-label="Mobile navigation">{navLinks.map(([label, href], index) => <DrawerClose key={href} render={<Link href={href} />} className={`flex min-h-12 items-center justify-between border-b border-primary-foreground/10 text-left text-sm font-bold uppercase tracking-[0.12em] transition-colors hover:text-accent ${path === href ? 'text-accent' : 'text-primary-foreground'}`}><span>{label}</span><span className="font-mono text-xs text-primary-foreground/40">0{index + 1}</span></DrawerClose>)}</nav>
          <DrawerFooter className="border-t border-primary-foreground/15 p-6"><DrawerClose render={<Link href="/contact" />} className="button-primary w-full">Start a conversation <ArrowUpRight data-icon="inline-end" /></DrawerClose></DrawerFooter>
        </DrawerContent>
      </Drawer>
    </div>
  </header>
...
export function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description?: string }) { return <Reveal className="mb-10 max-w-2xl"><p className="eyebrow text-accent">{eyebrow}</p><h2 className="mt-4 text-balance text-4xl font-black tracking-[-0.05em] md:text-5xl">{title}</h2>{description && <p className="mt-5 leading-7 text-muted-foreground">{description}</p>}</Reveal> }

export function ProjectCard({ project }: { project: (typeof projects)[number] }) { return <Reveal><Link href={`/projects/${project.slug}`} className="group block"><div className="media-slot aspect-[4/3] min-h-0"><span className="font-mono text-6xl font-bold text-primary/10">{project.accent}</span><span className="absolute bottom-4 left-4 text-xs font-bold uppercase tracking-[0.12em] text-primary/50">Asset slot · {project.name}</span></div><div className="flex items-start justify-between gap-5 border-b border-border py-5"><div><p className="eyebrow text-accent">{project.type}</p><h3 className="mt-2 text-2xl font-bold tracking-tight group-hover:text-accent">{project.name}</h3><p className="mt-2 text-sm text-muted-foreground">{project.location}</p></div><ArrowUpRight className="mt-1 shrink-0 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></div></Link></Reveal> }
